...
// an attacker host. The button validates https:// scheme + length first — AND,
// since the /mint/:url canonicalisation pass, a route param that isn't a tracked
// mint renders the "Not a tracked mint" state, so the latency button (and the
// whole probe-driven detail) never renders for an attacker URL at all.
...
test('a crafted non-mint route-param URL renders the not-tracked state and makes no request to the host', async ({ page }) => {
  ...
  // No probe-driven detail at all → no latency button to click.
  await expect(page.locator('.latency-test-btn')).toHaveCount(0)
